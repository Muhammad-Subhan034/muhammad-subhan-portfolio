 ...
 *  `cursor: none`) that snaps larger over anything interactive. Fine-pointer
  ...
  return (
    <>
      <div
        ref={dotRef}
        aria-hidden="true"
        className="pointer-events-none fixed left-0 top-0 z-[100] h-1.5 w-1.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-signal opacity-0 mix-blend-difference hidden md:block"
      />
      <div
        ref={ringRef}
        aria-hidden="true"
        className="pointer-events-none fixed left-0 top-0 z-[100] h-8 w-8 -translate-x-1/2 -translate-y-1/2 rounded-full border border-signal opacity-0 hidden md:block"
      />
    </>
  );
}
